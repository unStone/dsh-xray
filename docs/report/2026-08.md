...
| Kind | Count | Share |
|---|---|---|
| Installable plugin (declares dsh.bundle) | 6328 | 78% |
| Declares only dsh.client — not installable | 505 | 6% |
| No dsh integration found | 458 | 6% |
| A skill, not an installable plugin | 314 | 4% |
| Depends on dsh packages, no plugin manifest | 299 | 4% |
| Plugin code with no manifest to install it by | 168 | 2% |

**22% of repositories carrying the `dsh-plugin` topic are not installable plugins.**
<!--/STATS_TYPES-->
...
|---|---|---|---|
| **C3** | Powerful + sensitive behavior | 2310 | 29% |
| **C2** | Powerful (one of) | 4780 | 59% |
| **C1** | Ordinary | 592 | 7% |
| **C0** | No notable surface | 390 | 5% |

**88% of scanned plugins carry a powerful capability surface (C2+); 29% combine it with sensitive behavior (C3).**
<!--/STATS_LEVELS-->
...
<!--STATS_PATCH-->
**6299 plugins (78%) patch the dsh runtime itself via `manifest.bundle.patch`.**
...
<!--STATS_TOKEN-->
**639 plugins (8%) read credential-class environment variables** (names containing KEY/TOKEN/SECRET/PASSWORD), spanning AWS, Aliyun OSS, GitHub, and several model-provider APIs.
<!--/STATS_TOKEN-->
...
<!--FOOTER-->
*Data snapshot: 2026-08-20 · 8072 of 8690 repositories scanned · Top flags: `runtime_patch` 6299, `exec` 2279, `prompt_surface` 885, `token_env` 639, `exec_bundled` 547, `base64_decode` 524*
